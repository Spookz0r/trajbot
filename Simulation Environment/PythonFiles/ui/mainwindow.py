...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def on_enablecontrolbutton_clicked(self, checked):
        ...
        if checked:
            if self.simulationCheckBox.isChecked():
                ...
                try:
                    self.wlinkconnection = wlink.Wlink(str(self.portlineedit.text()),self.serialPrintbrowser) # create a Wlink object which communicates with the robot
                    self.controllooptimer.start(int(1./self.controlspinbox.value()*1000.)) # start timer for periodic execution of controlupdate function
                    self.last_controlupdate = time.time()
                except:
        ...
        if self.simulationCheckBox.isChecked():
            ...
            if pollResponseMocap[0][1]==select.POLLOUT: # check the availability of the mocap connection to the simulation(C program) 
                ...
                if pose[0] == pose[0]:
                    ...
                    self.animation.draw_robot(xs, ys, rots) # update the animation
    ...
    def parseNprint_SerialOutput(self,msg):
        ret_str = ''
        for line in msg.splitlines():
            msg_type=line[:3]
            if msg_type=='SYS':
                # System messages get disregarded
                continue
            elif msg_type=='OWN':
                # messages that yu want to print are of type OWN
                ret_str += line[4:]
                continue
            else:
                # Unkown types get disregarded
                continue
        if ret_str != '':
            self.logfileserial.write('%f; ' %(self.mocaptime) + ' ' + ret_str + '\n')
